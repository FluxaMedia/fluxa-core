...
fn key(item: &Value) -> Option<String> {
    ...
    item.get("progress_key").and_then(Value::as_str).map(str::trim).filter(|v| !v.is_empty()).map(str::to_string)
        .or_else(|| Some(match (item.get("season").and_then(Value::as_i64), item.get("episode").and_then(Value::as_i64)) {
...
fn resource_key(resource: &str, item: &Value) -> Option<String> {
    match resource {
        ...
        "library" => {
            ...
            (!id.is_empty() && !content_type.is_empty()).then(|| format!("{}:{id}", content_type.to_ascii_lowercase()))
        }
        "history" => {
            let id = item.get("content_id")?.as_str()?.trim();
            if id.is_empty() { return None; }
            Some(match (item.get("season").and_then(Value::as_i64), item.get("episode").and_then(Value::as_i64)) {
                (Some(season), Some(episode)) => format!("{id}_s{season}e{episode}"),
                _ => id.to_string(),
            })
        }
...
fn newer(left: &Value, right: &Value) -> bool {
    let left_at = left.get("last_watched").and_then(Value::as_i64).unwrap_or(i64::MIN);
    let right_at = right.get("last_watched").and_then(Value::as_i64).unwrap_or(i64::MIN);
    left_at > right_at || (left_at == right_at && left.get("position").and_then(Value::as_i64).unwrap_or(0) > right.get("position").and_then(Value::as_i64).unwrap_or(0))
}

fn deduplicate(items: Vec<Value>) -> Vec<Value> {
    let mut by_key = Map::new();
    for item in items {
        let Some(item_key) = key(&item) else { continue; };
        if by_key.get(&item_key).is_none_or(|previous| newer(&item, previous)) { by_key.insert(item_key, item); }
    }
    let mut values: Vec<Value> = by_key.into_values().collect();
    values.sort_by(|left, right| right.get("last_watched").and_then(Value::as_i64).cmp(&left.get("last_watched").and_then(Value::as_i64)));
    values
}
...
fn projection(items: &[Value]) -> Vec<Value> {
    ...
    for item in items {
        let id = item.get("content_id").and_then(Value::as_str).map(str::trim).unwrap_or("");
        let content_type = item.get("content_type").and_then(Value::as_str).map(str::trim).unwrap_or("");
        if id.is_empty() || content_type.is_empty() || item.get("duration").and_then(Value::as_i64).unwrap_or(0) <= 0 || item.get("position").and_then(Value::as_i64).unwrap_or(-1) < 0 { continue; }
        let content_key = format!("{}:{id}", content_type.to_ascii_lowercase());
        if latest.get(&content_key).is_none_or(|previous| newer(item, previous)) { latest.insert(content_key, item.clone()); }
    }
...
pub(crate) fn progress_sync_request_plan_json(args_json: &str) -> Option<String> {
    ...
    let initialized = state.and_then(|value| value.get("initialized")).and_then(Value::as_bool).unwrap_or(false);
    let cursor = state.and_then(|value| value.get("cursor")).and_then(Value::as_i64).unwrap_or(0).max(0);
    serde_json::to_string(&json!({ "mode": if initialized { "delta" } else { "bootstrap" }, "cursor": cursor })).ok()
}
...
pub(crate) fn apply_delta_sync_json(args_json: &str) -> Option<String> {
    ...
    let state = args.get("state").and_then(Value::as_object);
    let initialized = state.and_then(|value| value.get("initialized")).and_then(Value::as_bool).unwrap_or(false);
    let mut cursor = state.and_then(|value| value.get("cursor")).and_then(Value::as_i64).unwrap_or(0).max(0);
    let items = if initialized { state.and_then(|value| value.get("items")).and_then(Value::as_array).cloned().unwrap_or_default() } else { cursor = args.get("snapshotCursor").and_then(Value::as_i64).unwrap_or(cursor).max(0); args.get("snapshot").and_then(Value::as_array).cloned().unwrap_or_default() };
    let mut by_key: Map<String, Value> = items.into_iter().filter_map(|item| resource_key(resource, &item).map(|item_key| (item_key, item))).collect();
    let mut events = args.get("events").and_then(Value::as_array).cloned().unwrap_or_default();
    ...
    for event in events {
        ...
        if let Some(event_key) = resource_key(resource, &event) {
            match event.get("operation").and_then(Value::as_str).unwrap_or("").trim().to_ascii_lowercase().as_str() {
                "delete" => { by_key.remove(&event_key); }
                "upsert" => { by_key.insert(event_key, event); }
                _ => {}
            }
        }
        cursor = event_id;
    ...
    items.sort_by(|left, right| right.get("last_watched").or_else(|| right.get("watched_at")).or_else(|| right.get("added_at")).and_then(Value::as_i64).cmp(&left.get("last_watched").or_else(|| left.get("watched_at")).or_else(|| left.get("added_at")).and_then(Value::as_i64)));
...
pub(crate) fn apply_progress_sync_json(args_json: &str) -> Option<String> {
    ...
    let mut cursor = state.and_then(|value| value.get("cursor")).and_then(Value::as_i64).unwrap_or(0).max(0);
    let items = if initialized { state.and_then(|value| value.get("items")).and_then(Value::as_array).cloned().unwrap_or_default() } else { cursor = args.get("snapshotCursor").and_then(Value::as_i64).unwrap_or(cursor).max(0); args.get("snapshot").and_then(Value::as_array).cloned().unwrap_or_default() };
    let mut by_key: Map<String, Value> = deduplicate(items).into_iter().filter_map(|item| key(&item).map(|item_key| (item_key, item))).collect();
    let mut events = args.get("events").and_then(Value::as_array).cloned().unwrap_or_default();
    events.sort_by_key(|event| event.get("event_id").and_then(Value::as_i64).unwrap_or(0));
    for event in events {
        let event_id = event.get("event_id").and_then(Value::as_i64).unwrap_or(0);
...
mod tests {
    ...
    fn initialized_state_uses_delta_without_replacing_the_snapshot() {
        let request = progress_sync_request_plan_json(r#"{"state":{"initialized":true,"cursor":42}}"#).unwrap();
